...
public class MTRandom extends Random {
     ...
     * to calling genrand_int32. <p> If the compatibility flag is set to true,
     * then the algorithm will be seeded with the same default value as was used
     * in the original C code. Furthermore the setSeed() method, which must take
     * a 64 bit long value, will be limited to using only the lower 32 bits of
     * the seed to facilitate seamless migration of existing C code into Java
     * where identical behaviour is required. <p> Whilst useful for ensuring
     * backwards compatibility, it is advised that this feature not be used
     * unless specifically required, due to the reduction in strength of the
     * seed value.
     *
     ...
     * identical. <p> If this instance was initialised in 'compatibility' mode
     * then this method will only use the lower 32 bits of any seed value passed
     * in and will match the behaviour of the original C code exactly with
     ...
     * Random) we are able to replicate behaviour exactly. <p> Note that where
     * the number of bits requested is not equal to 32 then bits will simply be
     * masked out from the top of the returned integer value. That is to say
     * that:
     * <pre>
     * mt.setSeed(12345);
     ...
     * byte array each time setSeed() is called. <p> If the length of the byte
     * array is not a multiple of 4 then it is implicitly padded with zeros as
     * necessary. For example:
     * <pre>    byte[] { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06 }</pre> becomes
     * <pre>    int[]  { 0x04030201, 0x00000605 }</pre> <p> Note that this method
     * will not complain if the given byte array is empty and will produce an
     * empty integer array, but the setSeed() method will throw an exception if
     * the empty integer array is passed to it.
     *
